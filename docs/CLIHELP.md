...
Look below to find a thorough description of all command line options available when using Steamscraper. These options should be used with the `Steamscraper` command.
...
NOTE! If you've installed Steamscraper through the RetroPie-Setup script, it is recommended to create a symbolic link to the executable. Do this by running `sudo ln -s /opt/retropie/supplementary/steamscraper/Steamscraper /usr/local/bin/Steamscraper`. This will allow you to just type `Steamscraper` when running it from command line.

IMPORTANT! Most of the options can also be set in the `/home/USER/.steamscraper/config.ini` file thus removing the need to type them on command line all the time. Check the config.ini doc [here](CONFIGINI.md) for more info on this.
...
Running the following commands will scrape from all cached resources and generate a game list and composite artwork using the recipe in `/home/USER/.steamscraper/artwork.xml` (check the artwork documentation for more info on this [here](ARTWORK.md).
...
###### Example(s)
```
Steamscraper -p amiga -s openretro
Steamscraper -p snes -s screenscraper
Steamscraper -p amiga -s esgamelist
Steamscraper -p snes -s import
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper -u <userid:password>
```

### -i &lt;PATH&gt;
Sets the rom input folder. By default Steamscraper will look for roms in the `/home/<user>/RetroPie/roms/<PLATFORM>` folder. If your roms are located in a non-default location, you can set the input path using this option. Consider setting this in [`config.ini`](CONFIGINI.md#inputfolderhomepiretropieroms) instead.

###### Example(s)
```
Steamscraper -p snes -i "/path/to/your/snes/roms"
```
...
### -g &lt;PATH&gt;
Sets the game list export folder. By default Steamscraper exports the game list to the same directory as the rom input folder. This enables you to change that to a non-default location. Consider setting this in [`config.ini`](CONFIGINI.md#gamelistfolderhomepiretropieroms) instead.

###### Example(s)
```
Steamscraper -p snes -s screenscraper -g "/your/desired/game list/export/path"
```

### -o &lt;PATH&gt;
Sets the artwork / media output folder. By default Steamscraper outputs the composited artwork files to the game list export folder + `/media`. This allows you to change that to a non-default location. Read more about the artwork compositing [here](ARTWORK.md). Consider setting this in [`config.ini`](CONFIGINI.md#mediafolderhomepiretropieroms) instead.

###### Example(s)
```
Steamscraper -p snes -s screenscraper -o "/path/to/where/you/want/the/artwork/files"
```
...
```
Steamscraper -p snes -f attractmode -e snes
Steamscraper -p snes -f pegasus -e "/opt/retropie/supplementary/runcommand/runcommand.sh 0 _SYS_ snes"
```
...
###### Example(s)
```
Steamscraper -p snes -s thegamesdb -m 50
```
...
###### Example(s)
```
Steamscraper -p snes -l 500
```

### -c &lt;FILENAME&gt;
Sets a non-default config file. By default Steamscraper uses the file `/home/USER/.steamscraper/config.ini`.

###### Example(s)
```
Steamscraper -p snes -c "/path/to/config.ini"
```

### -a &lt;FILENAME&gt;
Sets a non-default xml file to use when setting up the artwork compositing. By default Steamscraper uses the file `/home/USER/.steamscraper/artwork.xml`. Read more about the artwork.xml format and customization options [here](ARTWORK.md). Consider setting this in [`config.ini`](CONFIGINI.md#artworkxmlartworkxml) instead.

###### Example(s)
```
Steamscraper -p snes -a "/path/to/artwork.xml"
```

### -d &lt;FOLDER&gt;
Sets a non-default location for the storing and loading of cached game resources. This is what is referred to in the docs as the *resource cache*. By default this folder is set to `/home/USER/.steamscraper/cache/<PLATFORM>`. Don't change this unless you have a good reason to (for instance if you want your cache to reside on a USB drive). The folder pointed to should be a folder with a Steamscraper `db.xml` file and its required subfolders inside of it (`covers`, `screenshots` etc.).

NOTE! If you wish to always use a certain location as base folder for your resource cache (for instance a folder on a USB drive), it is *strongly* recommended to set this in the config.ini file instead. Read more about the relevant config.ini option [here](CONFIGINI.md#cachefolderhomepisteamscrapercache).

###### Example(s)
```
Steamscraper -p snes -d "/custom/cache/path"
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --refresh
```

### --flags <FLAG1,FLAG2,...>
From Steamscraper 3.5.0 all command-line options that change the scraping behaviour have been combined into this option. Check below for a complete list of all the available flags and what they do. You can also get this list by using `--flags help`.
...
#### forcefilename
This flag forces Steamscraper to use the filename (excluding extension) instead of the cached titles when generating a game list. Consider setting this in [`config.ini`](CONFIGINI.md#forcefilenamefalse) instead.

NOTE! If 'nameTemplate' is set in config.ini this flag is ignored.
#### interactive
When gathering data from any of the scraping modules many potential entries will be returned. Normally Steamscraper chooses the best entry for you. But should you wish to choose the best entry yourself, you can enable this flag. Steamscraper will then list the returned entries and let you choose which one is the best one.
#### nobrackets
...
#### nohints
Disables the "Did you know" hints when running Steamscraper. Consider setting this in [`config.ini`](CONFIGINI.md#hintsfalse) instead.
#### nomarquees
...
#### noresize
By default, to save space, Steamscraper resizes large pieces of artwork before adding them to the resource cache. Adding this flag will disable this and save the artwork files exactly as they are retrieved from the scraping module. Consider setting this in [`config.ini`](CONFIGINI.md#cacheresizefalse) instead.
...
#### nosubdirs
By default Steamscraper will include roms located in subfolders. By adding this flag Steamscraper will only scrape the roms located directly in the input folder. See `-i <PATH>` above to read more about the rom input folder. Consider setting this in [`config.ini`](CONFIGINI.md#subdirstrue) instead.
#### nologos
...
#### onlymissing
This flag tells Steamscraper to skip all files which already have any piece of data from any source in the cache. This is useful if you just scraped almost all files from a platform succesfully with one source, and then want to only scrape the remaining games with a different source to fill in the holes. Normally Steamscraper will scrape all files again with the second source.
#### pretend
...
#### unattend
When generating a game list Steamscraper will check if it already exists and ask if you want to overwrite it. And it will also ask if you wish to skip existing game list entries. By using this flag Steamscraper will *always* overwrite an existing game list and *never* skip existing entries. This is useful when scripting Steamscraper to avoid the need for user input. Consider setting this in [`config.ini`](CONFIGINI.md#unattendtrue) instead.
#### unattendskip
When generating a game list Steamscraper will check if it already exists and ask if you want to overwrite it. And it will also ask if you wish to skip existing game list entries. By using this flag Steamscraper will *always* overwrite an existing game list and *always* skip existing entries. This is useful when scripting Steamscraper to avoid the need for user input. Consider setting this in [`config.ini`](CONFIGINI.md#unattendskipfalse) instead.
#### unpack
...
#### videos
By default Steamscraper doesn't scrape and cache video resources because of the significant disk space required to save them. You can enable videos using this flag. Consider setting this in [`config.ini`](CONFIGINI.md#videosfalse) instead.

###### Example(s)
```
Steamscraper -p amiga --flags forcefilename,nosubdirs,skipexistingwheels
Steamscraper -p nes --flags videos,nomarquees
```
...
NOTE! For any of these commands you can set a non-default resource cache folder with the `-d` option. The folder pointed to should be a folder with a Steamscraper `db.xml` file and its required subfolders inside of it (`covers`, `screenshots` etc.).
...
###### Example(s)
```
Steamscraper -p snes --cache edit <FILENAME 1> <FILENAME 2>
Steamscraper -p snes --cache edit --startat <FILENAME> --endat <FILENAME>
Steamscraper -p snes --cache edit:new=developer --startat <FILENAME> --endat <FILENAME>
Steamscraper -p snes --cache edit
Steamscraper -p snes --cache edit --fromfile "/home/pi/.steamscraper/reports/report-snes-missing_developer-20190708.txt"
Steamscraper -p snes --cache edit:new=ages --fromfile "/home/pi/.steamscraper/reports/report-snes-missing_ages-20190708.txt"
```
...
###### Example(s)
```
Steamscraper -p snes --cache merge:"path to/source/cache/snes"
Steamscraper -p snes --cache merge:"path to/source/cache/snes" -d "/path to/nondefault/destination/cache/snes"
```
...
###### Example(s)
```
Steamscraper -p snes --cache purge:all
Steamscraper -p snes --cache purge:m=thegamesdb
Steamscraper -p snes --cache purge:t=cover
Steamscraper -p snes --cache purge:m=thegamesdb,t=cover
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --cache refresh
```

#### --cache report:missing=&lt;all, textual, artwork, media or RESOURCE1,RESOURCE2,...&gt;
Will create report(s) containing all filenames of games missing the selected resource type(s). File(s) will be exported to `/home/USER/.steamscraper/reports/report-<PLATFORM>-missing_<RESOURCE>-yyyymmdd.txt`
...
NOTE! The reports can be fed back into Steamscraper using the `--fromfile <REPORTFILE>` option, which tells Steamscraper to only work on the files contained in the report. This is useful in combination with, for instance, the `--cache edit` option or the `--cache refresh`/`--refresh` (they are the same) option(s).

###### Example(s)
```
Steamscraper -p snes --cache report:missing=textual
Steamscraper -p snes --cache report:missing=artwork
Steamscraper -p snes --cache report:missing=developer
Steamscraper -p snes --cache report:missing=publisher,screenshot
```
...
###### Example(s)
```
Steamscraper -p snes --cache show
```
...
###### Example(s)
```
Steamscraper -p snes --cache vacuum
```
...
NOTE! This option doesn't clean up your game list media folders. You will need to do that yourself since Steamscraper has no idea what files you might keep in those folders. This option only relates to the resource cache database and related files.

###### Example(s)
```
Steamscraper -p snes --cache validate
```
...
```
Steamscraper -p snes --cache edit --fromfile "/home/pi/.steamscraper/reports/report-snes-missing_developer-20190708.txt"
Steamscraper -p snes -s screenscraper --fromfile "/home/pi/.steamscraper/reports/report-snes-missing_developer-20190708.txt"
```
...
NOTE 2! Instead of using this option, if you just want to gather resources for 1 or 2 roms, you can provide the filename(s) directly on the command like so: `$ Steamscraper -p snes -s thegamesdb "/full/or/partial/path/to/rom.zip"`. You can also use the `--fromfile` option.

###### Example(s)
```
Steamscraper -p snes --cache edit --startat "rom name.zip"
Steamscraper -p snes -s thegamesdb --startat "partial/path/to/rom name.zip"
```
...
###### Example(s)
```
Steamscraper -p snes --cache edit --endat "rom name.zip"
Steamscraper -p snes -s thegamesdb --endat "partial/path/to/rom name.zip"
```

### --includepattern &lt;PATTERN 1,PATTERN 2&gt;
Per platform Steamscraper have default file extensions that it will accept. This option allows you to only include certain files within that scope. The pattern is a simple asterisk type pattern. You can add several patterns by separating them with ','. In cases where you need to match for a comma you need to escape it as '\,' (see last example).
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --includepattern "Super*"
Steamscraper -p amiga --includepattern "*AGA*"
Steamscraper -p amiga --includepattern "*AGA*,Super*"
Steamscraper -p amiga --includepattern "*AGA*,Super*,*\, The"
```

### --excludepattern &lt;PATTERN 1, PATTERN 2&gt;
Per platform Steamscraper have default file extensions that it will accept. This option allows you to exclude certain files within that scope. The pattern is a simple asterisk type pattern. You can add several patterns by separating them with ','. In cases where you need to match for a comma you need to escape it as '\,' (see last example).
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --excludepattern "*[BIOS]*"
Steamscraper -p amiga --excludepattern "AGA*"
Steamscraper -p amiga --excludepattern "*AGA*,Super*"
Steamscraper -p amiga --excludepattern "*AGA*,Super*,*\, The"
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --includefrom "/home/pi/.steamscraper/includes.txt"
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --excludefrom "/home/pi/.steamscraper/excludes.txt"
```
...
###### Example(s)
```
Steamscraper -p snes -s thegamesdb --maxfails 75
```
...
```
$ Steamscraper -p <PLATFORM> -s <MODULE> --query "rick+dangerous" <FILENAME>
```
...
###### Example(s)
```
$ Steamscraper -p snes -s thegamesdb --query "rick+dangerous" /full/or/partial/path/to/rom.zip
$ Steamscraper -p snes -s screenscraper --query "md5=<CHECKSUM>" /full/or/partial/path/to/rom.zip
$ Steamscraper -p snes -s screenscraper --query "romnom=file%20name.zip" /full/or/partial/path/to/rom.zip
$ Steamscraper -p snes -s screenscraper --query "sha1=<CHECKSUM>&romnom=file%20name.zip" /full/or/partial/path/to/rom.zip
```
...
### --addext &lt;EXTENSION&gt;
If you have a rom that Steamscraper doesn't even try to gather data for, it might be because it has a file extension that isn't currently supported. This option allows you to temporarily add support for any file extension. If you feel like you are using a file extension that ought to be supported by default, please report it so it can be added in a later version of Steamscraper. Consider setting this in [`config.ini`](CONFIGINI.md#addextensionszst-smt) instead.

###### Example(s)
```
Steamscraper -p snes -s thegamesdb --addext *.ext
Steamscraper -p snes -s thegamesdb --addext "*.ext1 *.ext2"
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --lang es
```
...
###### Example(s)
```
Steamscraper -p snes -s screenscraper --region jp
```
...
### --verbosity &lt;0-3&gt;
Sets how verbose Steamscraper should be when running. Default level is 0. The higher the value, the more info Steamscraper will output to the terminal while running. Consider setting this in [`config.ini`](CONFIGINI.md#verbosity1) instead.

###### Example(s)
```
Steamscraper -p snes -s screenscraper --verbosity 3
```

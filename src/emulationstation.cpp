...
void EmulationStation::assembleList(QString &finalOutput, QList<GameEntry> &gameEntries)
{
	...
	for (auto &entry: gameEntries)
	{
		...
		else
		{
			...
			finalOutput.append("    <thumbnail>" +
			                   (config->relativePaths ? StrTools::xmlEscape(entry.coverFile).replace(config->inputFolder, ".") : StrTools::xmlEscape(entry.coverFile)) +
			                   "</thumbnail>\n");
		}
		...
		{
			finalOutput.append("    <image>" +
			                   (config->relativePaths ? StrTools::xmlEscape(entry.screenshotFile).replace(config->inputFolder, ".") : StrTools::xmlEscape(entry.screenshotFile)) +
			                   "</image>\n");
		}
		...
		{
			finalOutput.append("    <marquee>" +
			                   (config->relativePaths ? StrTools::xmlEscape(entry.marqueeFile).replace(config->inputFolder, ".") : StrTools::xmlEscape(entry.marqueeFile)) +
			                   "</marquee>\n");
		}
		...
		{
			finalOutput.append(
			    "    <desc>" + StrTools::xmlEscape(entry.description.left(config->maxLength)) + "</desc>\n");
		}
		if (entry.releaseDate.isEmpty())
		{
		...
		else
		{
			...
			finalOutput.append("    <releasedate>" + StrTools::xmlEscape(entry.releaseDate + (QRegularExpression("T[0-9]{6}$").match(entry.releaseDate).hasMatch() ? "" : "T000000")) +
			                   "</releasedate>\n");
		}
		if (entry.developer.isEmpty())
		{
...
QString EmulationStation::getLogosFolder()
{
...
QString EmulationStation::getIconsFolder()
{
	return config->mediaFolder + "/icons";
}

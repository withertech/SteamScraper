			...
			else if (line.left(1) != " " && line.left(1) != "\t" && line.contains(':'))
			{
				...
				else if (header == "file" || header == "files")
				{
					oldEntries.last().path = QString::fromUtf8(
					    line.right(line.length() - line.indexOf(":") - 1).trimmed());
				...
				{
					oldEntries.last().releaseDate = QDate::fromString(
				...
				{
					oldEntries.last().rating = QString::number(
					                               line.right(line.length() - line.indexOf(":") - 1).replace("%", "").toDouble() /
					                               100.0)
					                               .trimmed();
					currentPairValue = nullptr;// Don't allow multiline
				}
				...
				{
					oldEntries.last().logoFile = makeAbsolute(
					    QString::fromUtf8(line.right(line.length() - line.indexOf(":") - 1).trimmed()),
					    config->inputFolder);
					currentPairValue = nullptr;// Don't allow multiline
				}
...
void Pegasus::assembleList(QString &finalOutput, QList<GameEntry> &gameEntries)
{
	...
	*/
	...
	for (auto &entry: gameEntries)
	{
		...
		{
			finalOutput.append(
			    toPegasusFormat("rating", QString::number((int) (entry.rating.toDouble() * 100)) + "%") + "\n");
		}
		if (!entry.description.isEmpty())
		{
		...
		{
			finalOutput.append(toPegasusFormat("assets.screenshot",
			                                   (config->relativePaths ? entry.screenshotFile.replace(
			                                                                config->inputFolder, ".")
		...
		{
			finalOutput.append(toPegasusFormat("assets.boxFront",
			                                   (config->relativePaths ? entry.coverFile.replace(config->inputFolder,
			                                                                                    ".")
			                                                          : entry.coverFile)) +
			                   "\n");
		}
		...
		{
			finalOutput.append(toPegasusFormat("assets.marquee",
			                                   (config->relativePaths ? entry.marqueeFile.replace(config->inputFolder,
			                                                                                      ".")
			                                                          : entry.marqueeFile)) +
			                   "\n");
		}
		if (!entry.logoFile.isEmpty())
		{
			finalOutput.append(toPegasusFormat("assets.wheel",
			                                   (config->relativePaths ? entry.logoFile.replace(config->inputFolder, ".")
			                                                          : entry.logoFile)) +
			                   "\n");
		}
		if (!entry.videoFormat.isEmpty() && config->videos)
		{
			finalOutput.append(toPegasusFormat("assets.video",
			                                   (config->relativePaths ? entry.videoFile.replace(config->inputFolder,
			                                                                                    ".")
			                                                          : entry.videoFile)) +
			                   "\n");
		}
...
QString Pegasus::getLogosFolder()
{

...
void ScreenScraper::getSearchResults(QList<GameEntry> &gameEntries,
    ...
    for (int retries = 0; retries < RETRIESMAX; ++retries) {
        ...
        if (!jsonObj["response"].toObject()["ssuser"].toObject()["requeststoday"].toString().isEmpty() &&
            !jsonObj["response"].toObject()["ssuser"].toObject()["maxrequestsperday"].toString().isEmpty()) {
            reqRemaining = jsonObj["response"].toObject()["ssuser"].toObject()["maxrequestsperday"].toString().toInt() -
                           jsonObj["response"].toObject()["ssuser"].toObject()["requeststoday"].toString().toInt();
            if (reqRemaining <= 0) {
                printf("\033[1;31mYour daily ScreenScraper request limit has been reached, exiting nicely...\033[0m\n\n");
            }
        }
...
//    std::cout << QJsonDocument(jsonObj["medias"].toArray()).toJson().toStdString() << '\n';
    QString url = getJsonText(jsonObj["medias"].toArray(), REGION, QList<QString>({"steamgrid"}));
    if (!url.isEmpty()) {
        bool moveOn = true;
        for (int retries = 0; retries < RETRIESMAX; ++retries) {
            limiter.exec();
...
QList<QString> ScreenScraper::getSearchNames(const QFileInfo &info) {
    ...
    if (info.size() != 0) {
        searchNames.append("crc=" + hashList.at(1) + "&md5=" + hashList.at(2) + "&sha1=" + hashList.at(3) + "&romnom=" +

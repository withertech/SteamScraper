...
void Skyscraper::run()
{
	...
	printf("Icons folder:       '\033[1;32m%s\033[0m'\n", config.iconsFolder.toStdString().c_str());
	    ...
	    config.cacheOptions.contains("vacuum"))
	{
		...
		{
			success = cache->vacuumResources(config.inputFolder,
			                                 Platform::getFormats(config.platform, config.extensions,
	...
	QDir inputDir(config.inputFolder, Platform::getFormats(config.platform, config.extensions, config.addExtensions),
	...
	QDir iconsDir(config.iconsFolder);
	if (config.scraper == "cache" && !config.pretend)
		checkForFolder(iconsDir);
	config.iconsFolder = iconsDir.absolutePath();
...
void Skyscraper::loadConfig(const QCommandLineParser &parser)
{
	...
	if (settings.contains("cacheIcons"))
	{
		config.cacheIcons = settings.value("cacheIcons").toBool();
	}
	...
	if (parser.isSet("flags"))
	{
		if (parser.value("flags") == "help")
		{
			...
			printf("  \033[1;33mskipexistingicons\033[0m: When generating gamelists, skip processing icons that already exist in the media output folder.\n");
		...
		else
		{
			...
			for (const auto &flag: flags)
			{
				...
				else if (flag == "skipexistingicons")
				{
	...
	if (parser.isSet("noicons"))
	{
		config.cacheIcons = false;
	}
	...
	if (parser.isSet("skipexistingicons"))
	{
		config.skipExistingIcons = true;
	}
...
#if QT_VERSION >= 0x050a00
	printf("\033[1;33mDID YOU KNOW:\033[0m %s\n\n", hintsXml.elementsByTagName("hint").at(
	                                                                                      QRandomGenerator::global()->generate() % hintNodes.length())
	                                                    .toElement()
	                                                    .text()
	                                                    .toStdString()
	                                                    .c_str());
...
void Skyscraper::doPrescrapeJobs()
{
	    ...
	    config.scraper != "cache" && config.scraper != "import" && config.scraper != "esgamelist")
	{
		...
		netComm.request(
		    "https://raw.githubusercontent.com/HoraceAndTheSpider/Amiberry-XML-Builder/master/whdload_db.xml");
		q.exec();
	...
	else if (config.scraper == "igdb")
	{
		...
		if (updateToken)
		{
			    ...
			    jsonObj.contains("token_type"))
			{
				...
				{
					tokenFile.write(config.user.toUtf8() + ";" + config.igdbToken.toUtf8() + ";" +
					                QByteArray::number((QDateTime::currentMSecsSinceEpoch() / 1000) + tokenLife));
					tokenFile.close();
				}
			}
			else
